defmodule FusionFlowWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, FusionFlowWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      FusionFlowWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_sudo_mode" do
    ...
    test "redirects when authentication is too old", %{conn: conn, user: user} do
      fifteen_minutes_ago = DateTime.utc_now(:second) |> DateTime.add(-150, :minute)
      user = %{user | authenticated_at: fifteen_minutes_ago}
      user_token = Accounts.generate_user_session_token(user)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        FusionFlowWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
